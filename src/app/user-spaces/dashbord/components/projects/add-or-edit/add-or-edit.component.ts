...
export class AddOrEditComponent implements OnInit {
  ...
  // Image choice to display
  imageType = 'square';
  // File choice to display
  fileType = 'catalog';
  // class to color the buttons
  classButtonSquare = 'color-on';
  classButtonPicto = 'color-off';
  classButtonCatalog = 'color-on';
  classButtonBunch = 'color-off';
  ...
  uploadSquared(event: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = () => {
        image_url = event.target.result;
        img.src = reader.result as string;
        const image = document.getElementById('img-uploaded');
        image.innerText = '';
  ...
  public onImageTypeChange(value) {
    if (this.imageType === 'square' && value !== 'square') {
      this.imageType = 'picto';
      this.classButtonSquare = 'color-off';
      this.classButtonPicto = 'color-on';
      this.removeImage();
    } else if (this.imageType === 'picto' && value !== 'picto') {
      this.imageType = 'square';
      this.classButtonSquare = 'color-on';
      this.classButtonPicto = 'color-off';
    }
  }

  public onFileTypeChange(value) {
    if (this.fileType === 'catalog' && value !== 'catalog') {
      this.fileType = 'bunch';
      this.classButtonCatalog = 'color-off';
      this.classButtonBunch = 'color-on';
    } else if (this.fileType === 'bunch' && value !== 'bunch') {
      this.fileType = 'catalog';
      this.classButtonCatalog = 'color-on';
      this.classButtonBunch = 'color-off';
    }
  }

  checkNameLength(e) {
    const name_field = document.getElementById("name-field") as HTMLInputElement;
    const descr_name_field = document.getElementById("descr-name-field");
    var keynum;
    if (e.key) {
      keynum = e.key;
    }
    ...
    if (descr_name_field) {
      if (name_field.value.length === 24 && keynum != 'Backspace' && keynum != 'Delete' && keynum != 'Enter') {
        this.notis.warn('The name must not exceed more than 24 characters')
      }
      /* Display red message instead of 'Give a name to your project.'
      descr_name_field.classList.add('text-error');
      descr_name_field.textContent = "The name must not exceed more than 24 characters";
    }
    else {
      descr_name_field.classList.remove('text-error');
      descr_name_field.textContent = "Give a name to your project."
    }*/
    }
  }
  ...
  countNameLength() {
    const name_field = document.getElementById("name-field") as HTMLInputElement;
    const count_name = document.getElementById("count-name");

    var characterCount = name_field.value.length;

    count_name.textContent = characterCount.toString();
  }
}
